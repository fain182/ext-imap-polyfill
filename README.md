...
| `imap_fetch_overview` with `FT_UID` | a uid range wider than both the folder and 100,000 messages is refused, with `Sequence expands to more messages than any mailbox holds` on the error stack. The extension walks the mailbox rather than the range, so `1:4294967295` costs it nothing and would cost this package the memory for four billion numbers. The call answers `[]`, as it does for any other failure |
...
| `imap_setflag_full`, `imap_clearflag_full`, `imap_delete`, `imap_undelete`, `imap_mail_copy`, `imap_mail_move`, `imap_fetchbody`, `imap_savebody`, and `imap_open` over POP3 | an argument that goes on the wire unquoted — a flag, a message sequence, a body section, a POP3 user name or password — is refused when it holds a CR or LF instead of being sent: the line break ends the command and starts a second one, in a session that has already logged in. The extension sends the bytes. Each function then answers as it does for any other failure (`true` for the flag setters, `false` for the rest), with `Command argument contains a line break` on the error stack |
