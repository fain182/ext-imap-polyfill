...
What lands on the stack matters as much as when. A rejected command records the server's own response text — no tag, no status, no echo of the command — because that is what c-client hands `mm_log()`; `Imap\ImapEngineConnection` re-raises every rejected command as `CommandFailedException` for exactly that reason, and the same class fills the `imap_alerts()` stack from untagged `[ALERT]` responses (php_imap.c's `mm_notify`). Both overrides look like plumbing and are contract.

The same class re-raises the two ways a *stream* fails, for a different reason: the layer above acts on them rather than reporting them. `ConnectionLostException` is the server hanging up — `imap_ping()` answers it with false and an untouched stack, `imap_reopen()` answers it by dialling again — and `ConnectionFailedException` is everything else, which `imap_open()` reports as c-client's "Can't connect to host,port: reason" with the exception carrying the reason alone. Three outcomes, three types of this package's own — and no layer above `Connection\` names an ImapEngine class to tell them apart. Session did, until it didn't: a `catch` on a vendor exception class is the shape this leak takes, and it reads like plumbing at the catch site, so it is worth grepping for rather than trusting to review.
