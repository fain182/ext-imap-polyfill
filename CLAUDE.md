...
- **`src/Connection/Connection.php`** (`namespace IMAP`) — polyfill of the opaque native `IMAP\Connection` class. Sole owner of the backend. Knows nothing about imap_* contracts, ErrorStack, or return-value conventions. Two thirds of it is the emulated `MAILSTREAM`: cached message counts (`imap_num_msg` is a cached read that survives a dead connection, like c-client's `stream->nmsgs`), the selected folder and read-only bit, the registered keywords (`stream->user_flags`), half-open, expunge-on-close, and the pieces the reported `Mailbox` string is built from.
  - **Two routes to the wire, and which to take.** An operation whose behaviour depends on that state gets a named method here — `check()` and `selectOrExamine()` say nothing to a half-open connection. Everything else goes through `backend()`, which hands over the `ConnectionBackend` whole. Reaching a named operation through `backend()` instead bypasses the state, and has: a raw fetch of `BODYSTRUCTURE` went out where `fetchBodyStructure()` was waiting, and POP3 — which implements the named operation but not the raw item — silently got nothing back. When adding an operation, ask whether the stream's state changes it; if it does, it belongs here.
